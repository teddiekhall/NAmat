classdef NASearcher < handle
    ...
    methods
        ...
        function run(obj, par) 
            ...
            wb = waitbar(0, "NA Search - Initial Random Search");
                        
            % Initial search
            new_samples = obj.initial_random_search();
            obj.update_ensemble(new_samples, parallel);
            ...
            for iter = 1:obj.n
                ...
                if parallel == true && canUseParallelPool
                    run_parallel(obj, iter, inds, cells_to_resample)
                else 
                    run_serial(obj, iter, inds, cells_to_resample);
                end

                % Update progress bar
                waitbar(((iter+1)/(obj.n+1)), wb, "NA Search - Optimisation Loop")
            end
        ...
        function run_serial(obj, iter, inds, cells_to_resample)
            for i = 1:obj.nr
                ...
                new_samples = obj.random_walk_in_voronoi( ...
                    cells_to_resample(i, :), inds(i), stream_num);
                obj.update_ensemble(new_samples, false);
            end
        end
        ...
        function run_parallel(obj, iter, inds, cells_to_resample)
            ...
            parfor i = 1:obj.nr
                stream_num = (iter - 1) * obj.nr + i;
                samp_parts{i} = obj.random_walk_in_voronoi( ...
                    cells_to_resample(i, :), inds(i), stream_num);
            end
            
            % Assemble samp_parts into an array then update ensemble
            new_samples = zeros(obj.ns, obj.nd);
            for j = 1:obj.nr
                if j == 1
                    rows = 1:obj.nspnr + mod(obj.ns, obj.nr);
                else
                    rows = obj.nspnr * (j - 1) + mod(obj.ns, obj.nr) + (1:obj.nspnr);
                end
                new_samples(rows, :) = samp_parts{j};
            end

            obj.update_ensemble(new_samples, true);
        end
        ...
        function update_ensemble(obj, new_samples, parallel)
            ...
            if parallel == true
                new_objectives = zeros(size(new_samples,1), 1);
                parfor i = 1:size(new_samples, 1)
                    new_objectives(i) = objective_(obj, new_samples(i,:));
                end
            else
                new_objectives = arrayfun(@(n) objective_(obj, ...
                    new_samples(n,:)), 1:size(new_samples, 1))';
            end
